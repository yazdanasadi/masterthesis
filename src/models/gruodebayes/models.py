...
from src.models.gruodebayes.torchdiffeq import odeint
...
class NNFOwithBayesianJumps(torch.nn.Module):
    ...
    def forward(
        ...
        for i, obs_time in enumerate(times):
            ...
            if smoother:
                class_loss_vec[i_obs] += class_criterion(
                    self.classification_model(h[i_obs]), labels[i_obs]
                ).squeeze(1)
            ...
            loss_2 = loss_2 + compute_KL_loss(
                p_obs=p[i_obs], X_obs=X_obs, M_obs=M_obs, logvar=self.logvar
            )
        ...
        if smoother:
            class_loss_vec += class_criterion(
                self.classification_model(h), labels
            ).squeeze(1)
...
class GRUODEBayesSeq(torch.nn.Module):
    ...
    def forward(
        ...
        for i, obs_time in enumerate(times):
            ...
            h, loss_i, loss_pre = self.gru_bayes(
                h, X_obs, F_obs, i_obs, X=Xf_batch, M=Mf_batch
            )
