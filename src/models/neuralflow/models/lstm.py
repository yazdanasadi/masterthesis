...
from src.models.neuralflow.models import CouplingFlow, ODEModel, ResNetFlow
...
class BaseContinuousLSTM(Module):
    ...
    ]:  # Pre-jump state `h`, cell `c`, post-jump state `h`
...
class ContinuousLSTMLayer(Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if model == "ode":
            self.lstm = ODELSTM(
                dim, hidden_dim, activation, final_activation, solver, solver_step
            )
        elif model == "flow":
