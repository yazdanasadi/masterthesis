...
from src.models.linodenet.models.attention import MAB2 as MAB
import pdb
...
class Encoder(nn.Module):
    def __init__(
        ...
        for i in range(self.n_layers):
            self.channel_time_attn.append(
                MAB(nkernel, 2 * nkernel, 2 * nkernel, nkernel, self.nheads)
            )
            ...
            self.channel_attn.append(
                MAB(nkernel, nkernel, nkernel, nkernel, self.nheads)
            )
        self.relu = nn.ReLU()

    def gather(self, x, inds):
        # print(x.shape, inds.shape)
        inds2 = inds[:, :, None].repeat(
            1, 1, x.shape[-1]
        )  # keep repeating until the embedding len as a new dim
    ...
    def forward(self, context_x, value, mask, target_value, target_mask):
        ...
        C = (
            torch.ones([context_x.shape[0], ndims]).cumsum(1).to(self.device) - 1
        )  # BxC intialization for one hot encoding channels
        T_inds = (
            torch.cumsum(torch.ones_like(value).to(torch.int64), 1) - 1
        )  # BxTxC init for time indices
        C_inds = (
            torch.cumsum(torch.ones_like(value).to(torch.int64), -1) - 1
        )  # BxTxC init for channel indices
        ...
        # flattening to 2D
        T_inds_ = torch.stack(
            [pad(r[m]) for r, m in zip(T_inds, mk_bool)]
        ).contiguous()  # BxTxC -> Bxfull_len
        U_ = torch.stack(
            [pad(r[m]) for r, m in zip(value, mk_bool)]
        ).contiguous()  # BxTxC (values) -> Bxfull_len
        target_U_ = torch.stack(
            [pad(r[m]) for r, m in zip(target_value, mk_bool)]
        ).contiguous()  # BxK_
        target_mask_ = torch.stack(
            [pad(r[m]) for r, m in zip(target_mask, mk_bool)]
        ).contiguous()  # BxK_
        C_inds_ = torch.stack(
            [pad(r[m]) for r, m in zip(C_inds, mk_bool)]
        ).contiguous()  # BxK_
        mk_ = torch.stack(
            [pad(r[m]) for r, m in zip(mask, mk_bool)]
        ).contiguous()  # BxK_

        obs_len = torch.max(mk_.sum(-1)).to(
            torch.int64
        )  # K_max calculate max number of available obs
        ...
        C_ = torch.nn.functional.one_hot(C.to(torch.int64), num_classes=ndims).to(
            torch.float32
        )  # BxCxC #channel one hot encoding
        U_indicator = 1 - mk_ + target_mask_
        U_ = torch.cat(
            [U_[:, :, None], U_indicator[:, :, None]], -1
        )  # BxK_max x 2 #todo: correct
        ...
        for i in range(self.n_layers):
            ...
            U_ = self.relu(U_ + self.edge_nn[i](torch.cat([U_, k_t, k_c], -1))) * mk_[
                :, :, None
            ].repeat(1, 1, self.nkernel)
