...
def mTAN_collate(batch: list[Sample]) -> Batch:
    ...
    with torch.no_grad():
        for test_batch in test_loader:
            (
                ...
                observed_data,
                observed_mask,
            ) = (tensor.to(device) for tensor in test_batch)
            ...
            mse += (
                mean_squared_error(observed_data, pred_x, observed_mask)
                * observed_mask.sum()
            )

            test_n += observed_mask.sum()
    return mse / test_n
...
def compute_losses(dim, dec_train_batch, qz0_mean, qz0_logvar, pred_x, args, device):
    observed_data, observed_mask = (
        dec_train_batch[:, :, :dim],
        dec_train_batch[:, :, dim : 2 * dim],
    )

    noise_std = args.std  # default 0.1
    noise_std_ = torch.zeros(pred_x.size()).to(device) + noise_std
    noise_logvar = 2.0 * torch.log(noise_std_).to(device)
    logpx = (
        log_normal_pdf(observed_data, pred_x, noise_logvar, observed_mask)
        .sum(-1)
        .sum(-1)
    )
    pz0_mean = pz0_logvar = torch.zeros(qz0_mean.size()).to(device)
    analytic_kl = normal_kl(qz0_mean, qz0_logvar, pz0_mean, pz0_logvar).sum(-1).sum(-1)
    # pdb.set_trace()
    # if logpx
    if args.norm:
        logpx /= observed_mask.sum(-1).sum(-1) + 1e-8
        analytic_kl /= observed_mask.sum(-1).sum(-1) + 1e-8
...
def evaluate_classifier(
    ...
    for test_batch, label in test_loader:
        ...
        with torch.no_grad():
            ...
            if args.classify_pertp:
                ...
                _, label = label.max(-1)
                test_loss += (
                    nn.CrossEntropyLoss()(out, label.long()).item() * batch_len * 50.0
                )
            else:
                label = label.unsqueeze(0).repeat_interleave(num_sample, 0).view(-1)
                test_loss += (
                    nn.CrossEntropyLoss()(out, label).item() * batch_len * num_sample
                )
    ...
    auc = metrics.roc_auc_score(true, pred[:, 1]) if not args.classify_pertp else 0.0
...
def get_physionet_data(args, device, q, flag=1):
    ...
    if flag:
        test_data_combined = variable_time_collate_fn(
            test_data,
            device,
        ...
        if args.classif:
            ...
            print(
                train_data_combined[1].sum(),
                val_data_combined[1].sum(),
                test_data_combined[1].sum(),
            )
            print(
                train_data_combined[0].size(),
                train_data_combined[1].size(),
                val_data_combined[0].size(),
                val_data_combined[1].size(),
                test_data_combined[0].size(),
                test_data_combined[1].size(),
            )
...
def get_activity_data(args, device):
    ...
    test_data_combined = variable_time_collate_fn(
        test_data, device, classify=args.classif, activity=True
    )
    train_data, val_data = model_selection.train_test_split(
        train_data, train_size=0.8, random_state=11, shuffle=True
    )
    ...
    val_data_combined = TensorDataset(val_data_combined[0], val_data_combined[1].long())
    test_data_combined = TensorDataset(
...
def irregularly_sampled_data_gen(n=10, length=20, seed=0):
    ...
    for i in range(n):
        ...
        b = 10 * np.random.rand()
        f1 = (
            0.8 * np.sin(20 * (t1 + a) + np.sin(20 * (t1 + a)))
            + 0.01 * np.random.randn()
        )
        f2 = (
            -0.5 * np.sin(20 * (t2 + a + 20) + np.sin(20 * (t2 + a + 20)))
            + 0.01 * np.random.randn()
        )
        ...
        fg1 = 0.8 * np.sin(20 * (t + a) + np.sin(20 * (t + a)))
        fg2 = -0.5 * np.sin(20 * (t + a + 20) + np.sin(20 * (t + a + 20)))
        fg3 = np.sin(12 * (t + b))
...
def sine_wave_data_gen(args, seed=0):
    ...
    for _ in range(args.n):
        t = np.sort(
            np.random.choice(np.linspace(0, 1.0, 101), size=args.length, replace=True)
        )
        b = 10 * np.random.rand()
    ...
    combined_data = np.concatenate(
        (
            np.expand_dims(obs_values, axis=2),
            np.expand_dims(mask, axis=2),
            np.expand_dims(obs_times, axis=2),
        ),
        axis=2,
    )
    ...
    train_data, test_data = model_selection.train_test_split(
        combined_data, train_size=0.8, random_state=42, shuffle=True
    )
    ...
    test_dataloader = DataLoader(
        torch.from_numpy(test_data).float(), batch_size=args.batch_size, shuffle=False
    )
    data_objects = {
        "dataset_obj": combined_data,
        "train_dataloader": train_dataloader,
        "test_dataloader": test_dataloader,
        "input_dim": 1,
        "ground_truth": np.array(ground_truth),
...
def kernel_smoother_data_gen(args, alpha=100.0, seed=0, ref_points=10):
    np.random.seed(seed)
    obs_values, ground_truth, obs_times = [], [], []
    for _ in range(args.n):
        ...
        query_points = np.sort(
            np.random.choice(np.linspace(0, 1.0, 101), size=args.length, replace=True)
        )
        # query_points = np.sort(np.random.uniform(low=0.0, high=1.0, size=args.length))
        weights = np.exp(
            -alpha
            * (np.expand_dims(query_points, 1) - np.expand_dims(key_points, 0)) ** 2
        )
...
def get_toy_data(args):
    ...
    for i in range(dim):
        combined_obs_values[:, i, i * args.length : (i + 1) * args.length] = obs_values[
            :, i
        ]
        mask[:, i, i * args.length : (i + 1) * args.length] = 1.0
