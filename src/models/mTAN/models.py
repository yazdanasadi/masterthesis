...
class enc_mtan_classif(nn.Module):
    ...
    def time_embedding(self, pos, d_model):
        ...
        position = 48.0 * pos.unsqueeze(2)
        div_term = torch.exp(
            torch.arange(0, d_model, 2) * -(np.log(self.freq) / d_model)
        )
        pe[:, :, 0::2] = torch.sin(position * div_term)
        pe[:, :, 1::2] = torch.cos(position * div_term)
        return pe
    ...
    def forward(self, x, time_steps):
        ...
        if self.learn_emb:
            ...
            query = self.time_embedding(self.query.unsqueeze(0), self.embed_time).to(
                self.device
            )
...
class enc_interp(nn.Module):
    ...
    def attention(self, query, key, value, mask=None, dropout=None):
        ...
        bandwidth = torch.log(
            1 + torch.exp(self.bandwidth(torch.ones(1, 1, 1, 1).to(self.device)))
        )
...
class mTANDFull(nn.Module):
    def __init__(
        ...
        self.enc = enc_mtan_rnn(
            dim,
            torch.linspace(0, 1.0, num_ref_points),
            latent_dim,
            rec_hidden,
    ...
    def forward(self, timesteps, X, M, y_time_steps, test=False):
        ...
        pred_x = self.dec(
            z0,
            y_time_steps[None, :, :]
            .repeat(self.k_iwae, 1, 1)
            .view(-1, y_time_steps.shape[1]),
        )
