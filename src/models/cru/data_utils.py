...
from src.models.cru.pendulum_generation import generate_pendulums
from src.models.cru.physionet_preprocessing import download_and_process_physionet
from src.models.cru.ushcn_preprocessing import download_and_process_ushcn


class Batch(NamedTuple):
    r"""A single sample of the data."""

    obs: Tensor  # B×N:   the input timestamps.
    truth: Tensor  # B×N×D: the input values.
    obs_valid: Tensor  # B×N×D: the input mask.

    obs_times: Tensor  # B×K:   the target timestamps.
    mask_truth: Tensor  # B×K×D: the target values.
    mask_obs: Tensor  # B×K×D: teh target mask.
...
def cru_collate(batch: list[Sample]) -> Batch:
    ...
    for sample in batch:
        ...
        context_mask.append(torch.cat([mask_x, y_vals_temp], dim=0).to(torch.bool))
        # context_y = torch.cat([context_vals, context_mask], dim=2)

        target_vals.append(torch.cat([x_vals_temp, y], dim=0))
        target_mask.append(torch.cat([x_vals_temp, mask_y], dim=0).to(torch.bool))
        # target_y = torch.cat([target_vals, target_mask], dim=2)
    return Batch(
        obs=pad_sequence(context_vals, batch_first=True, padding_value=0).squeeze(),
        truth=pad_sequence(target_vals, batch_first=True, padding_value=0).squeeze(),
        obs_valid=pad_sequence(context_x, batch_first=True).squeeze(),
        obs_times=pad_sequence(context_x, batch_first=True).squeeze(),
        mask_truth=pad_sequence(target_mask, batch_first=True).squeeze(),
        mask_obs=pad_sequence(context_mask, batch_first=True).squeeze(),
    )
...
def load_data(args):
    ...
    if args.dataset == "pendulum":
        ...
        if args.task == "interpolation":
            if not os.path.exists(
                os.path.join(file_path, f"pend_interpolation_ir{args.impute_rate}.npz")
            ):
                print(f"Generating pendulum trajectories and saving to {file_path} ...")
                generate_pendulums(
                    file_path, task=args.task, impute_rate=args.impute_rate
                )

            train = Pendulum_interpolation(
                file_path=file_path,
                name=f"pend_interpolation_ir{args.impute_rate}.npz",
                mode="train",
                sample_rate=args.sample_rate,
                random_state=args.data_random_seed,
            )
            valid = Pendulum_interpolation(
                file_path=file_path,
                name=f"pend_interpolation_ir{args.impute_rate}.npz",
                mode="valid",
                sample_rate=args.sample_rate,
    ...
    # USHCN
        ...
        if not os.path.exists(
            os.path.join(file_path, "pivot_train_valid_1990_1993_thr4_normalize.csv")
        ):
            print(f"Downloading USHCN data and saving to {file_path} ...")
            download_and_process_ushcn(file_path)
        train_valid = USHCN(
            file_path=file_path,
            name="pivot_train_valid_1990_1993_thr4_normalize.csv",
        ...
        train = USHCN(
            file_path=file_path,
            name="pivot_train_1990_1993_thr4_normalize.csv",
            unobserved_rate=args.unobserved_rate,
            impute_rate=args.impute_rate,
            sample_rate=args.sample_rate,
        )
        valid = USHCN(
            file_path=file_path,
            name="pivot_valid_1990_1993_thr4_normalize.csv",
            unobserved_rate=args.unobserved_rate,
            impute_rate=args.impute_rate,
            sample_rate=args.sample_rate,
        )
        test = USHCN(
            file_path=file_path,
            name="pivot_test_1990_1993_thr4_normalize.csv",
            unobserved_rate=args.unobserved_rate,
            impute_rate=args.impute_rate,
            sample_rate=args.sample_rate,
        )
    ...
    # Physionet
        ...
        train_valid = Physionet(file_path=file_path, name="norm_train_valid.pt")
        train = Physionet(file_path=file_path, name="norm_train.pt")
        valid = Physionet(file_path=file_path, name="norm_valid.pt")
        test = Physionet(file_path=file_path, name="norm_test.pt")
    ...
    return train_valid_dl, train_dl, valid_dl, test_dl
...
class Pendulum_interpolation(Dataset):
    def __init__(self, file_path, name, mode, sample_rate=0.5, random_state=0):
        ...
        self.obs = np.ascontiguousarray(np.transpose(self.obs, [0, 1, 4, 2, 3])) / 255.0
        self.targets = (
            np.ascontiguousarray(np.transpose(self.targets, [0, 1, 4, 2, 3])) / 255.0
        )
...
class Pendulum_regression(Dataset):
    def __init__(self, file_path, name, mode, sample_rate=0.5, random_state=0):
        ...
        (
            ...
            train_time_points,
            test_time_points,
        ) = subsample(data, sample_rate=sample_rate, random_state=random_state)
...
class USHCN(Dataset):
    ...
    def __init__(
        ...
        self.data = (
            ...
            .sort_values(["UNIQUE_ID", "TIME_STAMP"])
            .set_index("UNIQUE_ID")
        )
        self.label_columns = self.data.columns[
            self.data.columns.isin([str(i) for i in columns])
        ]

    def __len__(self):
        return self.data.index.nunique()

    def subsample_time_points(
        self, sample, n_total_time_points, n_sample_time_points, seed=0
    ):
        rng = np.random.RandomState(seed)
        # try:
        #     choice = np.sort(rng.choice(n_total_time_points, n_sample_time_points, replace=False))
        # except ValueError:
        #     choice = np.sort(rng.choice(n_total_time_points, n_sample_time_points, replace=True))

        choice = np.sort(
            rng.choice(n_total_time_points, n_sample_time_points, replace=False)
        )
    ...
    def subsample_features(self, n_features, n_sample_time_points, seed=0):
        ...
        if self.unobserved_rate == "stratified":
            unobserved_mask = create_unobserved_mask(n_features, n_sample_time_points)

        # subsample features based on overall rate (most time points will have 1, 2 features missing, few will have more missing)
        elif isinstance(self.unobserved_rate, float) or isinstance(
            self.unobserved_rate, int
        ):
            assert (
                0 <= self.unobserved_rate < 1
            ), "Unobserved rate must be between 0 and 1."
            unobserved_mask = ~(
    ...
    def get_data_based_on_impute_rate(
        self, sample, unobserved_mask, n_features, n_sample_time_points
    ):
    ...
    def __getitem__(self, idx):
        ...
        unobserved_mask = self.subsample_features(n_features, n_sample_time_points)
...
class Physionet(Dataset):
    def __init__(self, file_path, name):
        self.data = torch.load(os.path.join(file_path, name), map_location="cpu")
...
def collate_fn_physionet(batch):
    ...
    time_points = [time_points for patient_id, time_points, obs, mask, label in batch]
    mask = [mask for patient_id, time_points, obs, mask, label in batch]

    obs = pad_sequence(obs, batch_first=True).to(device="cpu", dtype=torch.double)
