...
from src.models.cru.CRUCell import CRUCell, RKNCell
...
class CRULayer(nn.Module):
    ...
    def __init__(self, latent_obs_dim, args, dtype=torch.float64):
        ...
        self._cell = (
            RKNCell(latent_obs_dim, args, dtype)
            if args.rkn
            else CRUCell(latent_obs_dim, args, dtype)
        )
    ...
    def forward(
        ...
        for i in range(T):
            ...
            delta_t = (
                ...
                if time_points is not None and i < T - 1
                else torch.ones_like(latent_obs)[:, 0, 0]
            )
